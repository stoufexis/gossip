...
object Serde:
  ...
  /** @return
    ...
    */
  def encodeBounded(msg: OutgoingMessage, maxBytes: Int): (Set[Address], Chunk[Byte]) =
    val fieldsBytes: Chunk[Byte] = Codec.encode(Meta(msg.typ, msg.from, msg.to))

    val remainingSpace: Int = maxBytes - fieldsBytes.size

    /** Keeping bytes as a chunk of chunks will ensure that the length of the chunk will be added as the first
      * 4 bytes, as the Chunk encoder will be used.
      */
    @tailrec
    def loop(
      remainder: Chunk[Update],
      included:  Set[Address],
      bytes:     Chunk[Chunk[Byte]]
    ): (Set[Address], Chunk[Chunk[Byte]]) =
      if remainder.isEmpty then
        ...
        if bytes.size + bs.size > remainingSpace then
          ...
          loop(remainder.tail, included + next.address, bytes.appended(bs))
  ...
  def decodeAll(currentAddress: CurrentAddress): Chunk[Byte] => Either[String, Chunk[IncomingMessage]] =
    val destructuredDecoder: Decoder[(Meta, Chunk[Update])] =
